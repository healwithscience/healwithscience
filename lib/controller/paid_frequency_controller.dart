...
class PaidFrequencyController extends GetxController {
  final PaidFrequencyParser parser;
  PaidFrequencyController({required this.parser});
  ...
  Future<void> fetchPaidCategories() async {
    ...
    try {
      ...
      CollectionReference categoriesCollection = firestoreInstance.collection('biophotonic1');
  ...
  Future<void> savePurchaseFrequency(String categoryName) async {
    ...
    if (!documentExists) {
      ...
      await docReference.set({'purchasedFrequency1': purchasedFrequency});
      getPurchasedCategoryList();
      ...
      for (String item in purchasedFrequency) {
        docReference.update({'purchasedFrequency1': FieldValue.arrayUnion([item]),
        }).then((_) {
  ...
  Future<void> getPurchasedCategoryList() async {
    ...
    if (documentExists) {
      ...
      if (data != null && data.containsKey('purchasedFrequency1')) {
        purchasedCategory = List<String>.from(documentSnapshot['purchasedFrequency1']);
      }
